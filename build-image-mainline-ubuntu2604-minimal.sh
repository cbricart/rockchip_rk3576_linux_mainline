...
IMG_FILE="deploy/rk3576-photonicat2-mainline-ubuntu2604-minimal-${DATE_TS}.img"
...
BOOTFS_IMG_FILE="deploy/rk3576-photonicat2-mainline-ubuntu2604-minimal-bootfs.img"
ROOTFS_IMG_FILE="deploy/rk3576-photonicat2-mainline-ubuntu2604-minimal-rootfs.img"
...
if [ ! -f "kernel-6.18/deploy/Image" ]; then
    echo "Missing kernel image, build kernel first!"
    exit 3
fi

if [ ! -f "kernel-6.18/deploy/rk3576-photonicat2.dtb" ]; then
    echo "Missing kernel device tree, build kernel first!"
    exit 3
fi

if [ ! -f "kernel-6.18/deploy/kmods.tar.gz" ]; then
...
cp "${WORKDIR}/kernel-6.18/deploy/linux-"*.deb "${TMP_MOUNT_DIR}/rootfs/repo/"

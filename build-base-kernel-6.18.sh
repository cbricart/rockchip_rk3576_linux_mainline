...
KERNEL_VERSION="linux-6.18.38"
KERNEL_DIR="kernel-6.18"
KERNEL_PATCH_DIR="patches/kernel-6.18"
KERNEL_OVERLAY_DIR="patches/kernel-overlay-6.18"
...
if [ ! -d "${KERNEL_DIR}" ]; then
    ...
    mv "${KERNEL_VERSION}" "${KERNEL_DIR}"

    cd "${WORKDIR}/${KERNEL_DIR}"
    for i in "${WORKDIR}/${KERNEL_PATCH_DIR}/"*; do
        echo "Apply patch ${i}."
        patch --verbose -Np1 < "${i}"
    done
    cp -rf "${WORKDIR}/${KERNEL_OVERLAY_DIR}/." ./
...
mv -v *.deb deploy/
rm -rf "${WORKDIR}/${KERNEL_DIR}/deploy/modules" 2>/dev/null || true
rm -rf "${WORKDIR}/${KERNEL_DIR}/deploy/headers" 2>/dev/null || true
make O=build modules_install INSTALL_MOD_PATH="${WORKDIR}/${KERNEL_DIR}/deploy/modules" INSTALL_MOD_STRIP=1
make O=build headers_install INSTALL_HDR_PATH="${WORKDIR}/${KERNEL_DIR}/deploy/headers/usr/src/linux"
tar --owner=0 --group=0 --xform s:'^./':: -czf deploy/kmods.tar.gz -C "${WORKDIR}/${KERNEL_DIR}/deploy/modules" .
tar --owner=0 --group=0 --xform s:'^./':: -czf deploy/kheaders.tar.gz -C "${WORKDIR}/${KERNEL_DIR}/deploy/headers" .
cd "${WORKDIR}"
